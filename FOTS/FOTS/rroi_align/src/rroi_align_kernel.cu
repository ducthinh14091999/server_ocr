    ...
    scalar_t* con_idx_y)
{
    ...
    CUDA_KERNEL_LOOP(index, nthreads)
    {
        ...
        //TransformPrepare
        scalar_t roi_pooled_width = pooled_height * w / h;         // Different aspect ratio
        ...
        scalar_t M[2][3];                              // Rotation matrix
        M[0][0] = Alpha*Sx;
        M[0][1] = Beta*Sy;
        ...
        scalar_t P[8];                                 // Find the coordinates of 4 points in the original roi and 8 values
        ...
        // Find the center of the original rroi, and use bilinear interpolation to find f(x,y)
        ...
        scalar_t bin_cx = (leftMost + rightMost) / 2.0; // the center of rroi
        scalar_t bin_cy = (topMost + bottomMost) / 2.0;

        const bool in_rroi = pw <= roi_pooled_width;        // Is it within rroi?
        if (in_rroi){
