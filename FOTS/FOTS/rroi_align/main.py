...
roi_pooling_2d_fwd = cp.ElementwiseKernel(
            ...
            //pooled_weight
            int rois_pooled_width = (int)(ceil((float)(pooled_height * roi_width) / (float)(roi_height) ));          
            float bin_size_h = static_cast<float>(roi_height)  / static_cast<float>(pooled_height);                         // static_cast
...
plt.imshow(np.array(top_data.get())[0,0,:,:])
plt.show()
print(top_data)