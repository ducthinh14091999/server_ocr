...
class SynthTextDataset(Dataset):
    ...
    def __init__(self, data_root,
        ...
        self.wordBBoxes =[]
        self.transcripts =[]
        self.targetFilePath = glob.glob(self.dataRoot+'/*.h5')
        for data in  self.targetFilePath[:5]:
            ...
            for img in imnames:
                # self.imageNames.append(np.array(db['data'][img]))
                self.imageNames.append([data,img])
                self.wordBBoxes.append(db['data'][img].attrs['wordBB'])
                self.transcripts.append(np.array(db['data'][img].attrs['txt'],dtype=str))
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # im = image_path
            db = h5py.File(image_path[0])
            im = np.array(db['data'][image_path[1]])
            ...
            if num_of_words == len(transcripts):
                ...
                return '', image, score_map, geo_map, training_mask, transcripts, rectangles, rois
            else:
                return self.__getitem__(np.random.randint(0, len(self)))
