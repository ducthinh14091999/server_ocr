...
class GatedGCNNet(nn.Module):
    ...
    def forward(
        ...
        # input embedding
        h_embeding = self.embedding_h(h) #this is tensor of box
        e_embeding = self.embedding_e(e) #this is tensor of edge
    ...
    def _ohem(self, pred, label):
        ...
        # label = np.invert(label.astype(bool))
        pos_num = sum(label != 0)
        neg_num = pos_num * self.ohem

        pred_value = pred[:, 1:].max(1)

        neg_score_sorted = np.sort(-pred[label == 0])
    ...
    def loss(self, pred, label):
        ...
        m = nn.Softmax(dim=1)
        pred = m(pred)
        mask = torch.argmax(mask_label, 1, keepdim=False)
        # pred = pred.data.cpu().numpy()
